...
function Sidebar({ activeTab, setActiveTab, onBack }) {
  ...
  return (
    <div className="w-64 bg-surface/60 backdrop-blur-xl h-screen flex flex-col border-r border-white/5 pt-8">
      ...
      <nav className="flex-1 px-4 space-y-2 mt-2 relative">
        {menuItems.map((item) => {
          const isActive = activeTab === item.id;
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors duration-200 relative group z-10 ${isActive ? 'text-primary' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
              {isActive && (
                <motion.div
                  layoutId="activeTabIndicator"
                  className="absolute inset-0 bg-primary/10 border border-primary/20 rounded-xl shadow-[0_0_15px_rgba(99,102,241,0.2)] -z-10"
                  transition={{ type: "spring", stiffness: 300, damping: 25 }}
                />
              )}
              <item.icon size={20} className={`transition-all duration-300 ${isActive ? 'stroke-[2.5px] scale-110 drop-shadow-[0_0_8px_rgba(99,102,241,0.5)]' : 'stroke-2'}`} />

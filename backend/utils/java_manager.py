...
class JavaManager:
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        # Estrategia: solo 3 versiones de Java cubren todo
        # Java 8  -> Minecraft <=1.16.5 (APIs legacy, Forge antiguo no funciona con Java 9+)
        # Java 21 -> Minecraft 1.17–26.0 (backward compatible: Java 25 tambien sirve)
        # Java 25 -> Minecraft 26.1+     (class file v69, requiere Java 25 obligatorio)
        self.mc_java_requirements = {
            # Minecraft 26.1+ requiere Java 25
            "26.1": 25,
            "26.2": 25,
            "27": 25,
            # Minecraft 1.17–26.0 requiere Java 21 (minimum; Java 25 also works)
            "1.21": 21,
            "1.21.1": 21,
            "1.21.2": 21,
            "1.21.3": 21,
            ...
            "1.17.1": 21,
            "26.0": 21,
            # Minecraft 1.16.5 y anteriores requieren Java 8 (exactamente 8)
            "1.16": 8,
            "1.16.1": 8,
            "1.16.2": 8,
            "1.16.3": 8,
        ...
        if len(parts) >= 2:
            try:
                ...
                if major_version == 1:
                    ...
                    # NeoForge / Future Versioning
                    if minor_version >= 1 or major_version >= 27:
                        return 25
                    return 21
            ...
            logger.warning(
                f"Unknown Minecraft version {version}, defaulting to Java 21"
            )
            return 21
    ...
    def download_java(
        ...
        try:
            ...
            api_url = (
                f"https://api.adoptium.net/v3/assets/latest/{java_version}/hotspot"
            )
            params = {
                "vendor": "eclipse",
                "os": os_name,
            ...
            if not releases:
                logger.error(
                    f"No Java {java_version} releases found for {os_name}-{arch}"
                )
                return None
    ...
    def _download_file(
        self, url: str, path: Path, progress_callback: Callable[[float], None]
    ) -> bool:
    ...
    def _validate_java_install(self, java_path: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    f"Java validation failed (RC {result.returncode}): {result.stderr}"
                )
                return False
        except Exception as e:
    ...
    def _get_java_major_from_exe(self, java_path: str) -> Optional[int]:
        try:
            result = subprocess.run(
                [java_path, "-version"],
                capture_output=True,
    ...
    def _is_java_compatible(self, installed_major: int, required_version: int) -> bool:
        if required_version == 8:
            return installed_major == 8
        if required_version == 21:
            return installed_major >= 21
        if required_version == 25:
            return installed_major >= 25
        return installed_major >= required_version

    def get_java_for_server(
        self,
        server_path: str,
        ...
        try:
            ...
            logger.info(
                f"Looking for Java {required_version} for server at {server_path} (skip_download={skip_download})"
            )
            ...
            if not force_download and server_key in self.config["servers"]:
                ...
                cached_version = self.config["servers"][server_key].get("java_version")
                if Path(java_path).exists() and self._validate_java_install(java_path):
                    if cached_version == required_version:
                        ...
                        cached_major = self._get_java_major_from_exe(java_path)
                        if cached_major and self._is_java_compatible(
                            cached_major, required_version
                        ):
                            logger.info(
                                f"Cached Java {cached_major} is compatible with required {required_version}."
                            )
                            return java_path
                        logger.warning(
                            f"Cached Java version ({cached_version}) does not match required ({required_version}). Re-resolving."
                        )
            ...
            if java_dir.exists():
                ...
                if java_exe and java_exe.exists():
                    if self._validate_java_install(str(java_exe)):
                        ...
                        self._link_java_to_server(
                            server_path, minecraft_version, str(java_exe)
                        )
                        return str(java_exe)
                    else:
                        logger.warning(
                            "Local Java is corrupted. Deleting and re-downloading..."
                        )
                        try:
                            import shutil
                    ...
                    logger.warning(
                        "Local Java directory exists but executable missing. Deleting..."
                    )
                    try:
                        import shutil
            ...
            # 2b. If Java 21 is required but we have Java 25 locally, use it (backward compatible)
            if required_version == 21:
                java25_dir = self.base_dir / "java-25"
                if java25_dir.exists():
                    java25_exe = self._get_java_executable_path(java25_dir)
                    if (
                        java25_exe
                        and java25_exe.exists()
                        and self._validate_java_install(str(java25_exe))
                    ):
                        logger.info(
                            "Using locally installed Java 25 (compatible with Java 21 requirement)."
                        )
                        self._link_java_to_server(
                            server_path, minecraft_version, str(java25_exe)
                        )
                        return str(java25_exe)
                ...
                if system_java:
                    ...
                    if is_valid:
                        ...
                        if self._validate_java_install(path):
                            self._link_java_to_server(
                                server_path, minecraft_version, path
                            )
                            return path
                    else:
                        logger.info(
                            f"System Java {sys_ver} is not compatible with required Java {required_version}. Ignoring."
                        )

            # 4. Skip download if requested (during server start)
            if skip_download:
                logger.warning(
                    "skip_download=True, not downloading Java. User must install via wizard."
                )
                return None

            # 5. Descargar Java (Si llegamos aquí, no hay local ni system válido/suficiente)
            logger.info(
                f"Java {required_version} required for Minecraft {minecraft_version}, downloading..."
            )
            java_install_dir = self.download_java(
                required_version, progress_callback=progress_callback
            )
            ...
            if java_install_dir:
                ...
                if java_exe:
                    ...
                    if self._validate_java_install(java_path):
                        self._link_java_to_server(
                            server_path, minecraft_version, java_path
                        )
                        return java_path
                    else:
    ...
    def _link_java_to_server(
        self, server_path: str, minecraft_version: str, java_path: str
    ):
    ...
    def get_java_status(self, minecraft_version: str) -> Dict[str, any]:
        ...
        if not local_java_available and required_version == 21:
            java25_dir = self.base_dir / "java-25"
            if java25_dir.exists():
                java25_exe = self._get_java_executable_path(java25_dir)
                local_java_available = java25_exe and java25_exe.exists()
        ...
        if local_java_available:
            status["status_message"] = (
                f"✅ Java {required_version} available (local installation)"
            )
            ...
            status["status_message"] = (
                f"⚠️ Java {system_java[0]} found (Java {required_version}+ required)"
            )
            status["status_color"] = "orange"
            status["needs_download"] = True
        else:
            status["status_message"] = (
                f"❌ Java not found (Java {required_version}+ required)"
            )
